---
title: StudioShow
description: The Studio mode-switching surface has no Web implementation.
order: 3
section: components
element: StudioShow
category: studio
...
properties: [{"name":"kind","type":"string","default":"tune"},{"name":"track","type":"number","default":"0"}]
actions: []
catalog: 0.1.0
...
```dsx
<StudioShow kind="tune"/>
```

`StudioShow` takes no children.
...
- The kind attribute is not consumed on Web.

**Fallback.** Mounts the honest dsx-unsupported placeholder - a labelled marker carrying the `<StudioShow>` name and the dsx-unsupported class; never blank, never a fake lookalike of the native surface.
