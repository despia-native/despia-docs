---
title: LevelMeter
description: The Studio audio level meter has no Web renderer or audio-analysis seam.
order: 3
section: components
element: LevelMeter
category: studio
...
properties: [{"name":"level","type":"number","default":"0"}]
actions: []
catalog: 0.1.0
...
```dsx
<LevelMeter level="{{ dsx.variable.level }}"/>
```

`LevelMeter` takes no children.
...
- No visual-only placeholder is reported as functional metering.

**Fallback.** Mounts the honest dsx-unsupported placeholder - a labelled marker carrying the `<LevelMeter>` name and the dsx-unsupported class; never blank, never a fake lookalike of the native surface.
